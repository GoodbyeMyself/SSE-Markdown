    ...
    md.parse = function (src, env) {
        ...
        for (const tagName of customTags) {
            ...
            for (let i = matches.length - 1; i >= 0; i--) {
                ...
                const replacement = `<div class="custom-tag" data-tag-id="${chunkId}" data-tag-name="${tagName}" data-tag-complete="true" data-tag-content="${encodeURIComponent(
                    content
                )}"></div>`;

                modifiedSrc =
                    modifiedSrc.substring(0, index) +
                    replacement +
                    modifiedSrc.substring(index + fullMatch.length);
            }
